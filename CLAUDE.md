...
- **Auto-compact wake-up flow**: when context resumes after compaction, BEFORE taking any new action: (1) `ls docs/_meta/handovers/` to find the most recent `SESSION_HANDOVER_*.md` matching the active task; (2) Read it in full; (3) Then read the relevant `MEMORY.md` entries (Read `~/.claude/projects/-home-z00637938-workspace-easyr1-npu/memory/MEMORY.md` if not already in the system reminder). Auto-compact loses inline conversation context — handover + memory are the only durable channels. **Never plan around "this session" vs "next session" boundaries** — keep working until done; record decisions to versioned files so they survive compaction. See `memory/no_session_planning.md`.
