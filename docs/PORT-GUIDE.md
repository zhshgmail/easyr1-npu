...
| 我们的 fork | `zhshgmail/EasyR1` (GitHub, **private**) | 20 个 NPU port commit |
...
每个 archetype 的详细讲解 → [`knowledge/npu-patterns.md`](../knowledge/npu-patterns.md)（23 条 stable ID 目录）
...
# EasyR1 fork 的 ascend-port 分支（⚠️ 一定要指定 -b ascend-port）
git clone -b ascend-port https://github.com/zhshgmail/EasyR1.git
```

**如果 `zhshgmail/EasyR1` 是 private 且你没 access**：找项目 owner（zzcn2422）加 collaborator。或者他会把 fork 转 public / 镜像到 gitcode。
...
1. **立刻 grep [`knowledge/npu-patterns.md`](../knowledge/npu-patterns.md)** —— 23 条 stable ID 命中率很高
