...
- **发现一个 head-count 相关的精度失配**:**heads=4 时 tilelang 输出 44.3% 不对齐**(vs 它自己的 pytorch ref),heads=32 正常。这是候选 bug / 设计边界(待定:小 head 下 `padded_H`/`block_H`/`REPLICATE_H` tiling 是否算错,或 example 本就只为 heads≥某值设计)。**对 V4 相关**:V4-Flash 是 64 头(OK 档),但小 head 配置会咬人。
...
- **回答"要不要修 bug"**:R-KA-16 的 mitigation 已在,默认 config 路径干净;但 **heads=4 精度失配是一条需要查实/可能提 issue 的新发现**(下一步:判定真 bug vs 设计边界)。
