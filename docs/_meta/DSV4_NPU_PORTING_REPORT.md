...
| 实现 | 精度(vs pytorch 参考) | 性能 | 备注 |
|---|---|---|---|
| tilelang-ascend `sparse_mla_fwd` | **PASS**(rtol=5e-3 atol=1e-2,example 自带 pytorch ref) | **297.9 us/call** | B=1/seq=128/heads=32/dim=512+tail64/top_k=64,A3 实测;**未暴露 tilelang-ascend/AscendNPU-IR bug** |
| CANN `npu_nsa_select_attention` | 对比未成立 | (未取到有效数) | standalone harness 输入欠约束(随机 topk 无范围保证 → 退化 softmax 出 NaN);**这是 harness 问题,非 CANN 算子 bug**(之前以契约正确输入跑过 finite) |

**结论**:
- tilelang-ascend sparse_mla **精度 vs pytorch 对齐、性能 297.9us,表现正常** → 从此测试看 **tilelang-ascend MLIR / AscendNPU-IR 不需要额外修 bug**(R-KA-16 的 mitigation 已在;此 example 路径干净)。
- tilelang-vs-CANN 的**公平对比待补**:需给 CANN 侧搭输入契约正确(topk 范围/seqlen/mask 合规)的 harness 使其输出 finite,再同输入比精度+性能。当前 CANN 数不作数。
