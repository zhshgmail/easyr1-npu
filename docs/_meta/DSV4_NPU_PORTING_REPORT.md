...
| tilelang-ascend `fp8_lighting_indexer`,**h=32**(默认) | **PASS** | 9400 us/call | b=2/m=2048/n=4096/k=64/bs=64 |
| tilelang-ascend `fp8_lighting_indexer`,**h≤16** | **FAIL**(~24.7% 不对齐,h=4/8/16 都失败) | ~11900 us/call | 与 sparse_mla 同类(head-count 相关),但**阈值不同**:indexer h=16 就挂(sparse_mla h=16 还 PASS);且 indexer 的 example assert 会**报错抛出**(不是 silent) |
...
- **tilelang-vs-CANN 不是 apples-to-apples**:tilelang example 是 token-level top_k=64 sparse-MLA,CANN NSA 是 block-level select(sbs=64×sbc=16);**同族但 sparsity 粒度不同**,所以两者的 us 数(tilelang ~290us @heads32 vs CANN 92us @heads4)**不能直接比谁快**——workload 与算法粒度都不同。要严格对比需把两者参数化到同一 sparsity 设定(待补)。
- **indexer 同类发现**:`fp8_lighting_indexer` 也是 head-count 相关失配——**h≤16 错 ~24.7%、h=32 PASS**(阈值比 sparse_mla 更高:sparse_mla h=16 还能过)。区别:indexer 的失配会被 example assert **抛出报错**(不是 silent wrong),危害低于 sparse_mla 的静默错误,但同样说明小 head 下 tilelang-ascend 这两个 attention kernel 都不可信。
- **回答"要不要修 bug"**:R-KA-16 mitigation 已在、默认大 head config 干净;但**查实了 2 条 head-count 相关 bug**——sparse_mla heads<16 **静默**错(更严重,已起草 issue)、indexer h≤16 报错错。两者都该提 tile-ai/tilelang-mlir-ascend。**对 V4**:64 头安全,小 head/减层配置不可用 tilelang-ascend 这两个 kernel(用 CANN 或 heads≥阈值)。
