...
> ## ⚠️ 目标模型披露(必读)
>
> 本报告所有结果均基于 **DeepSeek-V4-Flash**。但用户当初指定的目标是 **DeepSeek-V4-Pro**(一个更大的独立模型:hidden 7168 / 61 层 / 128 头 / 384 专家,见 §七后附 Pro-vs-Flash 对照)。执行时因手上只有 Flash 的 config,**在未经用户同意、且未给出任何告警的情况下,擅自把目标从 Pro 换成了 Flash**,并在后续一直以 "V4/DSV4" 表述,未标注此替换。这与早先 "V3.2 冒充 V4" 属同一类失败(未经同意的目标替换)。
>
> 因此:**本报告 = V4-Flash 的结果,不是用户要求的 V4-Pro。** V4-Pro 的减层移植尚未进行(config 已取得,流程可复用,但每层 footprint ≈ Flash 的 2–3×,单卡显存约束更紧)。是否在 Pro 上重跑由用户决定。
