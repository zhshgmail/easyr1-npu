...
| ~~DEBT-3~~ | ~~install-skills.sh 没做 hooks 版本管理...~~ | ~~加 `.claude/.easyr1_hooks_version` 含 manifest_sha256...~~ | ✅ **完成**（commit pending）：install-skills.sh 写 `.claude/.easyr1_skills_version` 含 `_owner: easyr1-npu` + `manifest_sha256` + `deployed_count` + ISO timestamp；4 个 sanity tests 验证；OL-33 登记 | ~~1 h~~ |
...
| **DEBT-9** | M3 DSv4 sparse-attn 三个 native fused-grad（`npu_nsa_select_attention_grad` / compress-attention bwd / `npu_lightning_indexer_grad`）数值与 autograd 不一致：internal bwd P-reconstruction 与 fwd P 发散（select 经 11 probe + 源码 root-caused；indexer_grad 连调都 561103；`npu_nsa_compress_grad` 实为 pooling-grad 非 attn）。M3 改用 fwd-match-ref autograd fallback（数学正确、维度无关、2×2 验证），但比 fused-grad 慢 | 向 CANN NSA op owner 取 known-good grad 调用 / golden 或确认 op spec，再把 fallback 换回 native fused-grad 提速；证据：`FINDINGS_bwd_convention_2026-06-09.md`(89c939a) + `FINDINGS_compress_indexer_grad_2026-06-10.md`(28fc2d6) + scan 源码 pin | M3 训练吞吐需要 fused-grad 提速时 / CANN op owner ship 可验证 grad 或 known-good 调用时 | 2-4 h（RE 契约 + 重验，依赖 op owner 输入）|
