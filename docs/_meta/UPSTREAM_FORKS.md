...
| miles (RL framework, DSv4) | [`github.com/zhshgmail/miles`](https://github.com/zhshgmail/miles) | `npu-tilelang-ops` (`ff0161cc0`) | **2026-06-05 re-baseline (M1, 独立 agent 验证零 REFUTED)**: 分支落后 upstream `radixark/miles` main (`74198b45`) **115 commit**;NPU 工作在 `glm5/ops/_npu/` (sparse_mla/lighting_indexer fwd+bwd, is_npu idiom),**不含 DSv4 plugin** (DSv4 fork 点后才进 main、纯 GPU)。**决策: DSv4 NPU 运行层走 CANN-native** (torch_npu `npu_nsa_select_attention`/`npu_nsa_compress_attention`/`npu_lightning_indexer`/`npu_rms_norm`/`npu_mla_prolog_v3`, fwd+bwd 全覆盖已验), 不逐个 tilelang re-port。详见 `workspace/task-dag-realdelta/M1_LATEST_MILES_USECASE_UPSTREAM_MAP_2026-06-05.md` | (上游贡献候选 = DSv4 NPU dispatcher 路; M3 PoC 后 M5 评估) |
