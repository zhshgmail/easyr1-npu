...
| ID | Task | 状态 | 下一步 |
|---|---|---|---|
| T1-001 | `ascend-port` 两个 cherry-pick（`1f716ea` + `ecce71d`）在 8.5.0 生产 image 回归测 | **OPEN，P1** | 上 A3 跑 V1.4 smoke，数值对比 0.991 基线。见 [`HANDOVER.md §6.2`](HANDOVER.md) |
| T1-002 | `ascend-port` 在 8.5.2 drill image 跑 V1.4 smoke（把 "兼容两套 image" 从理论推到实测） | **OPEN，P1** | 同上，基准对比 drill 1.434 |
