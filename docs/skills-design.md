...
> 状态：V0.1，2026-04-18。初稿。参考 `~/workspace/a5/a5_ops/docs/design/SKILLS_DESIGN.md`（V3.1）的模式。本次移植 (EasyR1 → A3) 的经验作为该系统的第一个实例。
...
编号化 TODO：给 pattern 和 error correction 加上稳定 ID（比如 `NPU-CP-001` = nccl → hccl backend swap，`NPU-BUG-001` = triton-ascend 装漏 `__init__.py`），下一次引用就能 `@NPU-BUG-001` 点到。
...
2. 给 pattern / error correction 加 stable ID（`NPU-CP-NNN` / `NPU-BUG-NNN`）。
