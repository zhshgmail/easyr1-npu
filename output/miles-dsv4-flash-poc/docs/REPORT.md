...
```python
_FP8_WO_A_GEMM = envs.SGLANG_OPT_FP8_WO_A_GEMM.get()  # default True
```

**本 PoC 设置**:`SGLANG_OPT_FP8_WO_A_GEMM=0`(driver `_sglang_v4_minimal_PASS.py` 顶部),走 **bf16 路径**。同时关闭其他 16 个 OPT_* env(`SGLANG_OPT_USE_TILELANG_MHC_PRE/POST`、`SGLANG_OPT_USE_AITER_*`、`SGLANG_OPT_DEEPGEMM_HC_PRENORM`、`SGLANG_OPT_USE_FUSED_COMPRESS`、`SGLANG_OPT_USE_FUSED_QK_NORM_ROPE`、`SGLANG_OPT_USE_FUSED_CLAMP_ACT_MUL`、`SGLANG_OPT_USE_ONLINE_COMPRESS`、`SGLANG_OPT_USE_COMPRESSOR_V2`、`SGLANG_OPT_USE_OLD_COMPRESSOR`、`SGLANG_FIX_MTP_HC_HIDDEN`、`SGLANG_DSV4_FP4_EXPERTS`、`SGLANG_OPT_USE_TRITON_SWA_PREPARE`、`SGLANG_OPT_FUSE_WQA_WKV`),强制走 V4 model 的 torch fallback path。

**fp8 路径(本 PoC 没有 hit,因为 _FP8_WO_A_GEMM=0)**:
- `_fused_rmsnorm_fp8_quant`(`from aiter.ops.triton.fused_fp8_quant import fused_rms_fp8_group_quant`)
- `deep_gemm.tf32_hc_prenorm_gemm`(SM90+ specific)
- `deep_gemm.fp8_einsum`
- `sglang_per_token_group_quant_fp8`
- `_setup_fp8_wo_a_scales` + `transform_sf_into_required_layout`
- `_dequant_fp8_wo_a` 反向解量化路径(也由该 env 开关)

**bf16 路径(本 PoC 实际 hit,以下 14 个 gap 都在 bf16 路径上,与 fp8 无关)**:

| # | Gap | Site | path 类别 | PoC 修法 |
|---|---|---|---|---|
| 1 | `AscendAttnBackend._maybe_upgrade_forward_metadata` 缺失 | sglang NPU backend | NPU-side 缺 V4 hook(与 fp8/bf16 无关)| no-op stub |
| 2 | `sglang.srt.layers.mhc.hc_split_sinkhorn` 需要 tilelang/CUDA | sglang model bf16-side | bf16 path 上,但实现是 tilelang(NPU 不支持)| torch fallback(driver 内 stub)|
| 3 | `jit_kernel/dsv4/elementwise.fused_q_norm_rope` JIT 编译 CUDA C++ | sglang model bf16-side | bf16 path 上,但需要 nvcc(NPU 无)| torch fallback(elementwise.py 源码内)|
| 4 | `fused_rope_inplace` | sglang model bf16-side | 同上 | torch fallback |
| 5 | `fused_norm_rope_inplace`(`# Bf16-kv path used by the DSA prefill-CP case`,源码注释自证 bf16)| sglang model bf16-side | 同上 | torch fallback |
| 6 | `fused_k_norm_rope_flashmla`(写 FP8/BF16 packed page bytes 到 paged kvcache)| sglang model bf16-side(packed scheme 混 fp8+bf16) | bf16 path 上,但目标存储格式是 FP8/BF16 packed pages | PoC 跳过 scatter(1-layer + max_tokens=2 + dense attn 不需要 cache 回读)|
| 7 | NPU `aclnnIndex` 不支持 DT_COMPLEX64 dtype | torch_npu | 与 fp8 完全无关,torch_npu 层 gap | 在 real domain 索引,view-as-complex 后乘 |
| 8 | `AscendAttnBackend.forward_c4_indexer` 缺失 | sglang NPU backend | NPU-side 缺 V4 hook | no-op stub |
| 9 | `AscendAttnBackend.forward_core_compressor` 缺失 | sglang NPU backend | NPU-side 缺 V4 hook | no-op stub |
| 10 | `AscendAttnBackend.store_cache` / `forward_compress` / `init_forward_metadata_indexer` / `forward_compressor` 缺失 | sglang NPU backend | NPU-side 缺 V4 hook | no-op stubs |
| 11 | V4 dispatch 传 `compress_ratio` kwarg 给 `forward_extend`,NPU `forward_extend` 不接受 | sglang NPU backend | NPU-side V4 ABI 不匹配 | `**kwargs` 吸收 |
| 12 | `DeepSeekV4TokenToKVPool.get_key_buffer/get_value_buffer` 直接 raise `NotImplementedError`,但 NPU `forward_extend` 调用它 | sglang V4 KV pool API 与 NPU classic-MLA 互不兼容 | bf16 path 上,但是 V4 paged-MQA 与 V3.x classic-MLA 的存储/访问 API 完全不同 | V4 dense attention short-circuit 注入 `forward_extend`(直接用 fresh q,k,v) |
| 13 | `forward_decode` 同样收 `compress_ratio` + 同 KV pool 读取问题 | sglang NPU backend + V4 KV pool | 同上 | `**kwargs` 吸收 + V4 dense attention short-circuit |
| 14 | `jit_kernel/dsv4/moe.silu_and_mul_clamp` JIT 编译 CUDA C++ | sglang model bf16-side(fallback 分支)| bf16 path 上,但需要 nvcc | torch fallback(`F.silu(gate).clamp * up.clamp`)|
| 14+ | `jit_kernel/dsv4/gemm.linear_bf16_fp32` 用 `torch.mm(out_dtype=fp32)` 关键字 NPU 不支持 | sglang model bf16-side | bf16 path,但 NPU torch.mm 不接受 out_dtype | drop kw + `.float()` 转换 |

**结论 — fp8 vs missing-op 是正交的两个轴**:

1. **fp8/bf16 axis**(由 env `SGLANG_OPT_FP8_WO_A_GEMM` 控制):本 PoC 选 bf16,所有 fp8-only paths(`_FP8_WO_A_GEMM`、`deep_gemm.tf32_hc_prenorm_gemm`、`fp8_einsum`、`sglang_per_token_group_quant_fp8`)都不 hit。fp8 path 在 NPU 上还有额外的更难的问题(SM90+ kernel、deep_gemm 依赖、fp8 weight_scale_inv 等),但这不是当前 PoC 的负担。

2. **NPU missing-op axis**(与 fp8/bf16 无关):
   - 4 个 JIT-CUDA bf16 kernels(`fused_q_norm_rope` / `fused_rope_inplace` / `fused_norm_rope_inplace` / `silu_and_mul_clamp` / `fused_k_norm_rope_flashmla`)需要 **nvcc 编译 CUDA C++**,这是 nvcc/CUDA toolchain 层的 gap,不是算子 fp8/bf16 选择
   - 7 个 AscendAttnBackend V4 hook 缺失,与 fp8 完全无关 — 那是 NPU sglang adapter 还没跟上 V4 dispatch API 的新 hook
   - 1 个 torch_npu `aclnnIndex` 不支持 complex64 — torch_npu 算子 gap,与 fp8 无关
   - 1 个 V4 KV pool API 与 classic-MLA 不兼容 — V4 paged-MQA 设计上的 architectural mismatch

**所以 "DSv4 + sglang 的算子只支持 fp8 所以 NPU 跑不通" 是不准确的**:
- bf16 path 在 sglang V4 上 **存在并可达**(env 一开一关就切换)
