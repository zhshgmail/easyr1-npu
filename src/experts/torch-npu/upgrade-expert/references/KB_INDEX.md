...
- `easyr1-expert/references/PLATFORM_BUGS.md` — NPU-BUG-001..004 body (identical content; this expert's patterns/domains/ references the same)
- `easyr1-expert/references/SMOKE_BASELINE.md` — per-image V1.1/V1.3/V1.4 baselines; this expert uses the target-image row for V1.4 assertion
- `transformers/upgrade-expert/references/patterns/domains/dockerfile-target.md` — sibling Dockerfile template; very similar structure but targets the full stack swap, not just torch
