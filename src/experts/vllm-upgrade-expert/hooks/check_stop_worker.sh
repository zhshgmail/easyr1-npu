...
#   CLAUDE_AGENT_NAME        — must equal 'vllm-upgrade-worker' or hook is a no-op
...
# workspace/vllm-upgrade-*/ and find the one with latest PROGRESS.md mtime.
#
# Exit codes:
#   0  all invariants OK
...
err() { echo "[check_vllm_upgrade_worker] $*" >&2; }

# --- locate the agent's workspace ---
WS_ROOT="$PROJECT_DIR/workspace"
WS=""
if [[ -d "$WS_ROOT" ]]; then
  # newest vllm-upgrade-* dir with a PROGRESS.md
  WS=$(find "$WS_ROOT" -maxdepth 2 -type f -name PROGRESS.md -path '*/vllm-upgrade-*/*' -printf '%T@ %p\n' 2>/dev/null \
    | sort -nr | head -1 | awk '{print $2}' | xargs -I{} dirname {} 2>/dev/null || true)
fi

if [[ -z "$WS" || ! -d "$WS" ]]; then
  err "no vllm-upgrade-* workspace with PROGRESS.md found under $WS_ROOT"
  err "worker must create workspace/vllm-upgrade-{SESSION_TAG}/PROGRESS.md in P0"
  exit 4
fi

PROGRESS="$WS/PROGRESS.md"
echo "[check_vllm_upgrade_worker] workspace: $WS"
...
if [[ -n "$EDITED_FILES" ]]; then
  echo "[check_vllm_upgrade_worker] G2: static_check on $(echo "$EDITED_FILES" | wc -l) files"
...
if grep -qiE '\b(PASS|green|closed|end.to.end|complete)\b' "$PROGRESS"; then
  echo "[check_vllm_upgrade_worker] G3: PASS-style claim found in PROGRESS.md, verifying evidence"
...
echo "[check_vllm_upgrade_worker] OK — G2 + G3 + OL-09 all satisfied"
exit 0
