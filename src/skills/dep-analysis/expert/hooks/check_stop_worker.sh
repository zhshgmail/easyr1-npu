#!/usr/bin/env bash
# Generic Stop hook for NPU-port worker agents.
#
# REFERENCE COPY. Each expert forks this into its own hooks/ and fills in:
#   - CLAUDE_AGENT_NAME expected value (the expert's worker name)
#   - WS_ROOT workspace-directory-pattern (expert-specific prefix)
#   - caller-package — the thing dry-import validates
#
# Enforces the universal G2 / G3 / OL-04b / OL-09 invariants as documented
# in _shared/references/ALWAYS_LOADED_UNIVERSAL.md.
#
# Inputs (from Claude Code hook env):
#   CLAUDE_AGENT_NAME        — must equal 'dep-analysis-worker' or hook is a no-op
#   CLAUDE_PROJECT_DIR       — repo root
#   CLAUDE_SESSION_ID        — used to derive workspace dir, if known
#   CLAUDE_TRANSCRIPT_PATH   — JSONL of the agent run (for edit extraction)
#
# Fallback: if env vars missing, walk recent sessions under
# workspace/dep-analysis-*/ and find the one with latest PROGRESS.md mtime.
#
# Exit codes:
#   0  all invariants OK
#   2  BLOCKING — G2 static_check failed (py_compile / dry-import)
#   3  BLOCKING — G3 PASS claim without log evidence
#   4  BLOCKING — PROGRESS.md missing required provenance fields (OL-09)
#   10 hook internal error (bad env, missing tooling) — treat as BLOCKING too

set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
EXPERT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
PROJECT_DIR="${CLAUDE_PROJECT_DIR:-$(cd "$EXPERT_ROOT/../../../.." && pwd)}"

# --- gate: only run for the worker agent ---
AGENT="${CLAUDE_AGENT_NAME:-}"
if [[ -n "$AGENT" && "$AGENT" != "easyr1-port-worker" ]]; then
  exit 0
fi

err() { echo "[check_dep_analysis_worker] $*" >&2; }

# --- locate the agent's workspace ---
WS_ROOT="$PROJECT_DIR/workspace"
WS=""
if [[ -d "$WS_ROOT" ]]; then
  # newest easyr1-port-* dir with a PROGRESS.md
  WS=$(find "$WS_ROOT" -maxdepth 2 -type f -name PROGRESS.md -path '*/dep-analysis-*/*' -printf '%T@ %p\n' 2>/dev/null \
    | sort -nr | head -1 | awk '{print $2}' | xargs -I{} dirname {} 2>/dev/null || true)
fi

if [[ -z "$WS" || ! -d "$WS" ]]; then
  err "no dep-analysis-* workspace with PROGRESS.md found under $WS_ROOT"
  err "worker must create workspace/dep-analysis-{SESSION_TAG}/PROGRESS.md in P0"
  exit 4
fi

PROGRESS="$WS/PROGRESS.md"
echo "[check_dep_analysis_worker] workspace: $WS"

# --- OL-09: PROGRESS.md provenance fields (dep-analysis-specific) ---
for field in "MODE:" "CONSUMER_REF:" "CANDIDATE_IMAGE:" "Handoff:"; do
  if ! grep -q "^$field" "$PROGRESS" 2>/dev/null; then
    err "OL-09 violation: PROGRESS.md missing required field '$field'"
    exit 4
  fi
done

# --- OL-04b: cleanup field must be present and non-empty ---
# Accept: "Cleanup: clean", "Cleanup: partial", "Cleanup: skipped <reason>".
# Reject: missing line, or bare "Cleanup: skipped" without a reason.
CLEANUP_LINE=$(grep -E '^Cleanup:' "$PROGRESS" 2>/dev/null | head -1 || true)
if [[ -z "$CLEANUP_LINE" ]]; then
  err "OL-04b violation: PROGRESS.md missing 'Cleanup:' field. Run scripts/cleanup_session.sh."
  exit 4
fi
case "$CLEANUP_LINE" in
  "Cleanup: clean"*|"Cleanup: partial"*) ;;
  "Cleanup: skipped "?*) ;;  # skipped WITH reason
  "Cleanup: skipped"|"Cleanup: skipped "*)
    err "OL-04b violation: 'Cleanup: skipped' requires a concrete reason after 'skipped'."
    exit 4
    ;;
  *)
    err "OL-04b violation: 'Cleanup:' value must be 'clean', 'partial', or 'skipped <reason>'. Got: $CLEANUP_LINE"
    exit 4
    ;;
esac

# --- G2: dep-analysis-expert is read-only by design ---
# OL-08 restricts writes to $WORKSPACE only. If any .py file got edited
# outside $WORKSPACE, that's a severe OL-08 violation (not merely a
# py_compile issue). Detect and reject.
TRANSCRIPT="${CLAUDE_TRANSCRIPT_PATH:-}"
if [[ -n "$TRANSCRIPT" && -f "$TRANSCRIPT" ]]; then
  OUT_OF_SCOPE=$(python3 -c "
import json, sys, os
WS = os.environ.get('CLAUDE_PROJECT_DIR','') + '/workspace/'
try:
    with open('$TRANSCRIPT') as f:
        for line in f:
            try:
                rec = json.loads(line)
            except Exception:
                continue
            msg = rec.get('message') or {}
            for blk in (msg.get('content') or []):
                if not isinstance(blk, dict): continue
                if blk.get('type') != 'tool_use': continue
                if blk.get('name') not in ('Edit','Write','MultiEdit'): continue
                fp = (blk.get('input') or {}).get('file_path') or ''
                if fp and 'dep-analysis-' not in fp:
                    print(fp)
except Exception as e:
    sys.stderr.write(f'transcript-parse-failed: {e}\n')
" 2>/dev/null | head -5 || true)
  if [[ -n "$OUT_OF_SCOPE" ]]; then
    err "OL-08 violation: dep-analysis-worker is read-only outside workspace. Edits found:"
    echo "$OUT_OF_SCOPE" | sed 's/^/  /' >&2
    exit 2
  fi
fi
echo "[check_dep_analysis_worker] G2/OL-08: no out-of-workspace edits detected"

# --- G3: P1/D=0/no-blockers claim must cite the dep-gap-report with at least one classified dep ---
if grep -qiE '\b(scenario:?\s*P1|D=0|no blockers|analysis green|PASS|complete|classification)\b' "$PROGRESS" 2>/dev/null; then
  echo "[check_dep_analysis_worker] G3: classification claim found, verifying evidence"
  # Require dep-gap-report.md referenced + the file exists
  REPORT_REF=$(grep -oE 'dep-gap-report\.md' "$PROGRESS" | head -1)
  if [[ -z "$REPORT_REF" ]]; then
    err "G3 BLOCKING: classification claim in PROGRESS.md without a dep-gap-report.md reference"
    exit 3
  fi
  REPORT_PATH="$WS/dep-gap-report.md"
  if [[ ! -f "$REPORT_PATH" ]]; then
    err "G3 BLOCKING: dep-gap-report.md referenced but file missing at $REPORT_PATH"
    exit 3
  fi
  # Require at least one specific dep cited in the report (A/B/C/D/E row format)
  if ! grep -qE '^\s*-\s+\S+:\s+[ABCDE]\b' "$REPORT_PATH" 2>/dev/null && \
     ! grep -qE '\|\s+\S+\s+\|\s+[ABCDE]\s+\|' "$REPORT_PATH" 2>/dev/null; then
    err "G3 BLOCKING: dep-gap-report.md has no classified-dep lines (expect '- pkg: A/B/C/D/E' or a markdown table)"
    exit 3
  fi
fi

echo "[check_dep_analysis_worker] OK — OL-09 + G2(read-only) + G3(report-evidence) satisfied"
exit 0
