...
#   CLAUDE_AGENT_NAME        — must equal 'transformers-day0-worker' or hook is a no-op
...
# workspace/transformers-day0-*/ and find the one with latest PROGRESS.md mtime.
#
# Exit codes:
#   0  all invariants OK
...
err() { echo "[check_transformers_day0_worker] $*" >&2; }

# --- locate the agent's workspace ---
WS_ROOT="$PROJECT_DIR/workspace"
WS=""
if [[ -d "$WS_ROOT" ]]; then
  # newest transformers-day0-* dir with a PROGRESS.md
  WS=$(find "$WS_ROOT" -maxdepth 2 -type f -name PROGRESS.md -path '*/transformers-day0-*/*' -printf '%T@ %p\n' 2>/dev/null \
    | sort -nr | head -1 | awk '{print $2}' | xargs -I{} dirname {} 2>/dev/null || true)
fi

if [[ -z "$WS" || ! -d "$WS" ]]; then
  err "no transformers-day0-* workspace with PROGRESS.md found under $WS_ROOT"
  err "worker must create workspace/transformers-day0-{SESSION_TAG}/PROGRESS.md in P0"
  exit 4
fi

PROGRESS="$WS/PROGRESS.md"
echo "[check_transformers_day0_worker] workspace: $WS"

# --- OL-09: PROGRESS.md provenance fields (transformers-upgrade-specific set) ---
for field in "MODE:" "TARGET_TRANSFORMERS_VERSION:" "BASE_IMAGE:" "UPSTREAM_REF:" "Handoff:"; do
...
if [[ -n "$EDITED_FILES" ]]; then
  echo "[check_transformers_day0_worker] G2: static_check on $(echo "$EDITED_FILES" | wc -l) files"
...
if grep -qiE '\b(PASS|green|closed|end.to.end|complete)\b' "$PROGRESS"; then
  echo "[check_transformers_day0_worker] G3: PASS-style claim found in PROGRESS.md, verifying evidence"
...
echo "[check_transformers_day0_worker] OK — G2 + G3 + OL-09 all satisfied"
exit 0
