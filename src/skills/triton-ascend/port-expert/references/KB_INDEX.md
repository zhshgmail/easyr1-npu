...
| 3 | 2026-04-28 T25 cold-drive replay — re-validate case 2 from clean state | n/a — same vendor wheel + smoke script | **6/6 PASS reproduced**. P0 dry-run: `git merge-base --is-ancestor v3.6.0 origin/main` correctly returns 1 (vendored-fork sanity check passes). Re-`git merge v3.6.0 --no-commit --no-ff` returns "Already up to date." on `ascend-port/triton-v3.6.0`. Smoke ran on chip `/dev/davinci2` with `ASCEND_RT_VISIBLE_DEVICES=0`. Note: triton 3.2.0's `torch.empty_like` emits a `Cannot create tensor with internal format while allow_internel_format=False` UserWarning — non-blocking, expected on torch_npu 2.9.0+cpu. Skill+KB are reproducible by a fresh agent. |
