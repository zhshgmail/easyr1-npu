# Domain — vllm API drift families (cross-version)

> **Load when**: Phase B classification points at `C-patch` and the root
> cause is a vllm community API change (not an NPU/torch ABI issue).
...
> specific vllm versions, SHAs, or file paths**; those belong in
> `KB_INDEX.md §"Known vllm API removals / adds"`.
...
sig-sniff** so the same vllm-ascend tree can be built against **both**
the old and new vllm (vllm-ascend policy: one tree serves both the
stable and nightly vllm). Hard version gates like
`if vllm.__version__ >= "0.20"` are **last resort** — they force a
release fork. Prefer duck-typing / try-import / hasattr.

## What this file is NOT

- Not a list of "here's the fix for vllm 0.20.0 PR #32951". That
  belongs in `KB_INDEX.md §"Known vllm API removals / adds"` as a
  dated entry pointing back to the family.
- Not a replacement for the worker running the failure minimization
  flow in `vllm-ascend-probe.md`. You still need to isolate the
  smallest repro before classifying; classify too early and you
  may match the wrong family.
