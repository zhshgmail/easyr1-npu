...
| 46 | lighting_indexer_fwd (P1.5) | `examples/deepseek_v4/example_lighting_indexer_fwd_kernel.py` | Developer | PASS — max abs err 0.000000 vs fp32 CPU ref @ SEQ=8, SKV=16, H=8, D=32, BN=16, BQ=4 | T33: NPU port of miles `tilelang_indexer_fwd.py` (both glm5 + V4 variants share this kernel). Discovered R-KA-7 (no per-scalar GM stores) + R-KA-8 (explicit slice ranges) the hard way |
| 47 | lighting_indexer_bwd (P1.6) | `examples/deepseek_v4/example_lighting_indexer_bwd_kernel.py` | Developer | **PASS at SEQ=1** — dQ/dW/dKV all err 1e-5 vs PyTorch autograd. NaN in dKV at SEQ≥2 (R-KA-14 OPEN); workaround: per-batch-element invocation matches miles wrapper anyway. | T33: P1.6. Full pipeline working in production pattern. SEQ≥2 grid-block reset bug R-KA-14 OPEN. |
...
| **R-KA-7: Never write to global memory per-scalar inside an inner loop; always write tile-level via `T.copy(shared_buf, GM[start:end, ...])`** | T33.P1.5: `for bn,bq: Logits[seq_i+bq, k*BN+bn] = scores_shared[bn,bq]` compiled OK but trapped at NPU runtime with `aicore exception: MPU address access is invalid` (cube_error=0, mte_error=non-zero → MTE = memory transfer engine, not cube). Fix: transpose to a fragment, T.copy to a shared buffer, then one `T.copy(buf, GM[range])` does the tile store. | Lint: in `examples/*.py`, grep for `Output\[.*\] *= *.*\[.*\]` inside `T.serial`/`T.Parallel`; flag any global-buffer assignment in inner loops; require tile-level T.copy patterns. |
| **R-KA-8: Slices into kernel-argument Tensors MUST be explicit ranges `[start:end, ...]`; single-index forms `IndexQ[seq_i * H, 0]` that work on CUDA fail the MLIR verifier with "expected `tensor<MxN>` or rank-reduced version (size mismatch)"** | T33.P1.5: my initial port copied upstream's `T.copy(IndexQ[seq_i * heads, 0], index_q_shared)` verbatim; mlir-ascend rejected it with a size-mismatch verifier error because the slice extent didn't fall out from a single index on rank-2 input. Explicit `IndexQ[seq_i * heads : seq_i * heads + block_Q * heads, 0:index_dim]` lowers correctly. | Rule for porters: when adapting an upstream kernel that copies via single-index forms (typical CUDA pattern), rewrite every `T.copy(SRC[a, b], DST)` and `T.copy(SRC, DST[a, b])` to use the full `[start:end]` range on every dim, computed from the in-scope `block_*` size. |
| **R-KA-9: Avoid `for h: dst[h, d] = src[0, h, d]` element-by-element transpose/reshape loops; use rank-reducing slice T.copy instead — `T.copy(IndexQ[bx, 0:H, 0:D], q_shared)`** | T33.P1.6: manual element loops to squeeze [1,H,D]→[H,D] lowered to `scf.for + tensor.extract + tensor.insert`, which `bishengir-compile` rejected with `error: 'tensor.empty' op Unsupported op for finding the root alloc.` + `error: 'hivm.hir.load' op Unsupported op for finding the root alloc.` Replacing with a single `T.copy(SRC[scalar_idx, slice, slice], 2D_dst)` (rank-3 input rank-reduced to 2D dst) lowers cleanly. | Lint: in `examples/*.py`, grep for `for [a-z_]+ in T\.serial.*\n.*for [a-z_]+ in T\.serial.*\n.*\w+\[[^\]]+\] = .*\[[^\]]+\]`; flag these patterns; require `T.copy(slice, slice)` instead. |
| **R-KA-10: `T.atomic_addx4(dst[idx, off], src[i, off])` MUST pass `size=[4]` explicitly; otherwise only 1 element fires per call.** | T33.P1.6 + T33.P1.4: `_get_extent(dst)` on a single-index access returns `[1]`, so `T.atomic_addx4` fires for only 1 element. With `size=[4]`, all four elements scatter. Symptom: `dKV[i, 0:4]` shows `[v, 0, 0, 0]` instead of `[v0, v1, v2, v3]`. | Lint: grep `T\.(npuir_)?atomic_addx4\(.*\)` and require `size=` keyword arg. |
| **R-KA-11: For ReLU-gated bwd, `gated = grad * weights * mask` where `mask = 1{scores > 0}`; NEVER `gated = grad * scores_relu * weights`** | T33.P1.6: my initial code used `(grad * scores_relu) * weights`, which is the dW path (correct for dWeights[h] = sum_k grad[k] * scores_relu[k,h]). For dQ and dK we need `grad * weights * mask` — multiplying by `scores_relu` adds an extra factor that scales gradient by relu output magnitude instead of by indicator function. Symptom: dQ values ~20-25x too small. Compute mask in NPU as `min(scores_relu * BIG, 1.0)` with BIG=1e6 (avoid overflow at 1e10). | Code review: when porting a relu-gated bwd, verify the gradient formula at each output. dW = grad * scores_relu (uses value), dQ/dK = grad * weights * mask (uses indicator). |
| **R-KA-12: `out_idx=[-N]` for the JIT decorator auto-allocates the Nth-from-end argument and returns it — but the auto-alloc shape inference may produce wrong rank/dtype if the kernel only writes a partial slice per block. Prefer to drop `out_idx` and have callers pre-allocate ALL outputs.** | T33.P1.6: `out_idx=[-2]` against a per-block writeback `T.copy(buf, dIndexQ[bx, 0:H, 0:D])` produced an auto-alloc with shape `(H, H)` and dtype fp32 instead of the expected `(seq, H, D)` fp16 — the JIT shape inference seemingly mistook the slice pattern for the whole tensor. Removing `out_idx` and passing all outputs explicitly fixed both. | Pattern: for any kernel with per-grid-block partial output writes (typical for scatter/gather patterns), declare all outputs as kernel args and let the caller pre-allocate. |
| **R-KA-13 (E5 WORKAROUND VERIFIED 2026-05-19): `T.vsub(acc_after_gemm, broadcast_buf, acc_after_gemm)` silently zeros when `broadcast_buf` is filled OUTSIDE the same pipelined iteration as the gemm feeding it.** | T33.P1.4 sparse_mla_bwd. Originally diagnosed: `vsub(acc_dp, delta_frag, acc_dp)` produces all-zero output while `vsub(acc_p, lse_expanded, acc_p)` earlier in the same kernel works. The crucial distinguishing detail (not caught in the first round of workarounds): the working lse vsub fills `lse_expanded[h,b]=lse_frag[h,0]` via `for h_i in T.serial(...): for bi_i in T.serial(...): ...` **inside the inner pipelined loop body, immediately preceding the vsub**. The failing experiments built `delta_expanded` outside the pipeline iter or via vbrc, both of which lower differently. **E5 WORKAROUND**: replicate the working lse construction exactly — Python-fill `delta_expanded[h_i, bi_i] = delta_frag[h_i, 0]` inside the inner loop right before `T.vsub`. Result on sparse_mla_bwd: dQ cosine 0.5255→0.9276; max abs err 0.0988→0.0389 (rel 0.585; still has bf16+scaling magnitude drift to investigate, but direction is production-grade). | Whenever a `vsub(acc, broadcast)` silently zeros, the schedule-locality of the broadcast-buf construction matters: place the scalar-fill **inside the same inner pipelined iteration, immediately before the vsub**. The bishengir lowering treats schedule-adjacency as register-layout consumption order. The same pattern likely applies to `vmul/vadd` with broadcast operands following a gemm. |
| **R-KA-14 (OPEN): NPU multi-block kernels with atomic_addx4 scatter produce NaN in the second+ grid block.** | T33.P1.6 lighting_indexer_bwd: at SEQ=1 (single grid block), dQ/dKV/dW all match autograd to 1e-5. At SEQ≥2, grid block 1+ produce NaN in dKV regardless of which kv indices they write to (tested with disjoint and overlapping index ranges). Hypothesis: shared-buffer or fragment-register stale state across grid blocks (`alloc_shared`/`alloc_fragment` lifetimes unclear across `T.Kernel` grid iterations on NPU). Workaround: call the kernel per-batch-element (matches miles' `batched_indexer_bwd` per-batch loop, so this is the natural shipping pattern). | Lint/audit: for kernels that use `T.Kernel(N, is_npu=True)` with N>1 grid blocks AND `atomic_addx4`/`atomic_add` scatter, smoke test at N=1 and N=2 separately. If results diverge at N=2, surface state likely isn't reset between blocks. |
| **R-KA-15 (OPEN): `atomic_addx4` with all-zero source buffer produces garbage values (magic constant 6e37) instead of being a no-op.** | T33.P2 lighting_indexer_bwd corner case: when the input Q tensor has all-non-positive values (e.g. all 0 or all -0.1), scores_relu = max(Q@K^T, 0) = 0 → mask = 0 → gated = grad * w * 0 = 0 → d_k = gated @ Q = 0. The atomic_addx4(dKV[idx], d_k_shared[i, 0:4], size=[4]) call should be a no-op (adding 0 to dKV). Instead it writes ~6e37 magnitude values. Confirmed deterministic: `all zeros` and `all -0.1` produce IDENTICAL garbage value `60405883577979550923416661670064291840.000000`. Skipping the atomic_addx4 call makes dKV stay 0 correctly. Reproduced in minimal kernel at standalone P1.6 level. | Workaround (wrapper-side): before calling the bwd kernel, host-side check if any input combination would lead to all-zero scores_relu (e.g. if all grad_scores are 0, OR if you can statically prove all scores ≤ 0). Skip the kernel call entirely. Lint/audit: any kernel doing atomic_addx4 in a "may-have-all-zero-src" path needs a wrapper-side short-circuit. |
