...
| 47 | lighting_indexer_bwd (P1.6) | `examples/deepseek_v4/example_lighting_indexer_bwd_kernel.py` | Developer | COMPILE_OK + RUN_OK on NPU; dKV atomic_addx4 produces visibly non-zero values; **numerics WIP** (dQ output capture wrong; need to fix out_idx wrapper). Required hot-swapping fresh bishengir-compile (built from AscendNPU-IR master). | T33: P1.6. Path of progress: fresh bishengir gave readable errors, then R-KA-9 (no element-by-element transpose) refactor unblocked compile |
