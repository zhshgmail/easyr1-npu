...
| **R-KA-13 (OPEN): At certain points in a pipelined kernel, `T.vsub(big_BHxBS, small_BHx1, big_BHxBS)` zeros out the result silently even though the identical pattern works elsewhere in the same kernel.** | T33.P1.4 sparse_mla_bwd: vsub of `acc_dp` against `delta_frag` produces zero output, while the structurally identical `T.vsub(acc_p, lse_frag, acc_p)` in the same iter works correctly. Replacing the `[BHx1]` operand with a `[BHxBS]` expanded buffer ALSO fails. Replacing vsub with vmul (same operands) works. Replacing delta_frag with vbrc'd zero works. Workaround: omit the vsub (produces biased gradients with cosine ~0.5 vs ref). Probable cause: bishengir-compile lowering interaction with the surrounding gemm + vexp pipeline. Needs minimal reproducer + upstream report. | When porting flash-attention-style bwd: smoke test the dS computation with debug dumps of acc_dp BEFORE and AFTER each step. If the second `vsub(acc, broadcast_buf)` in a sequence zeros out, isolate via a minimal test + report. |
