...
| **T2** | 跟进 tile-ai PR #80 review | IN-PROGRESS | — | T1, T3, T4 | 看 reviewer 反馈,如要 C++ port 转化、调注释,响应。已修 CI:`d2d1871` 解 3 个 ruff F841(weave biggest_name + biggest_block_M_guess 进诊断文案),删 stale `tvm.script.tir` import,ruff format 全过;5/5 CheckUBBudget UT cold cache on A3 PASS。等下一轮 reviewer 反馈。 → artifact:PR #80 merged 或 next-revision commit |
...
| **T9** | 提 `Ascend/AscendNPU-IR` **issue update + 诊断报告**(原"上游 PR")| TODO | T6 | 在 issue #251 加一条 update comment(或新 issue),包含:(a) T5 dump 方法 + 配套驱动 `dump_rka16_ir.py` 链接 (b) T6 bisect 表(311 passes × iter_args count)+ 突变点 `ExtendedCanonicalizer` (c) before/after IR diff 显示 `acc_m`/`acc_l` 被吃 (d) 建议方向:在 `bishengir/lib/Transforms/ExtendedCanonicalizer.cpp` 加 pattern 黑名单或 DPS in-place iter_arg 保留规则 (e) 附 `rka16_ns4_pass_index.txt`。**不带 Claude/CC 签名,用 gc CLI**。→ artifact:issue #251 update URL 或新 issue URL |
