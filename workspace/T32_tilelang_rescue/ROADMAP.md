...
| **T12** | 4 个上游 PR 整理 + 提交 | IN-PROGRESS | T10, T11 | 4 个 PR 拆解:**(1) `tile-ai/tilelang-mlir-ascend` PR #80** 已开,CI 全绿,MERGEABLE,等 reviewer。**(2) `Ascend/AscendNPU-IR`**:T9 已用 issue #251 comment 路径替代上游 PR(Huawei 偏好 issue 沟通)。**(3) Megatron-LM 同名问题不在 `NVIDIA/Megatron-LM` 上游 main**,只在 `radixark/Megatron-LM` miles-main 分支(单 commit `23924a0`);本地 branch `fix/te_general_gemm_npu_fallback` 备好 commit `6f3209b`(8 行 patch),等用户确认 fork 目标。**(4) `radixark/miles` clean PR 分支已准备好**:新分支 `npu-tilelang-ops`(off `f7960bb` upstream main tip,1 commit `2629705`),从 18-commit dev branch 蒸出 13 文件 1767 LOC 核心 `_npu/` 子包 + dispatch hook,**剔除 e2e 驱动 / smoke / validation 等下游 dev 工具**;已 push 到 fork `zhshgmail/miles npu-tilelang-ops`,PR body draft 在 `/tmp/miles_pr_body.md`;等用户确认开 PR。无 Claude 签名,gc/gh CLI 发。→ artifact:4 PR URL |
