...
| **T12** | 4 个上游 PR 整理 + 提交 | IN-PROGRESS | T10, T11 | 4 个 PR 拆解:**(1) `tile-ai/tilelang-mlir-ascend` PR #80** 已开,CI 全绿,MERGEABLE,等 reviewer。**(2) `Ascend/AscendNPU-IR`**:T9 已用 issue #251 comment 路径替代上游 PR(Huawei 偏好 issue 沟通)。**(3) Megatron-LM PR 撤销 + 重路由**:用户 2026-05-28 指出 NPU Megatron 适配走 `Ascend/MindSpeed`(`mindspeed/megatron_adaptor.py` import-time monkey-patch `features_manager`),**不直接 patch Megatron-LM**。grep 验证:MindSpeed 已 patch `TEColumnParallelLinear/TERowParallelLinear/TEDotProductAttention/TELayerNormColumnParallelLinear/TE*GroupedLinear` 但**未** patch `te_general_gemm`。我的 `6f3209b` 基于 MindSpeed-bypass 路径(tlrescue 容器没装 MindSpeed),**不算充分验证**,本地保留待 MindSpeed-aware 复现后决定真 PR target(若 MindSpeed install 后 import OK → 撤;若仍 NameError → PR 到 `Ascend/MindSpeed features_manager`,加 `pm.register_patch('megatron.core.transformer.moe.moe_utils.te_general_gemm', None, create_dummy=True)`)。详 memory `feedback_npu_megatron_via_mindspeed.md`。**(4) `radixark/miles` PR 分支 `npu-tilelang-ops` commit `d03db2c` 已 audit-cleaned(13 文件 1767 LOC)**,但 **充分验证条件未满足**:T11 e2e 是在 MindSpeed-bypass 路径上的,开 PR 前先在 tlrescue 装 MindSpeed 重跑确认 4 算子 + 52M-param Megatron e2e 仍 PASS。无 Claude 签名,gc/gh CLI 发。→ artifact:4 PR URL |
