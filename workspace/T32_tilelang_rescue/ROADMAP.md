...
| ID | 任务 | 状态 | deps | 详细 |
|---|---|---|---|---|
| **T13** | 用 Huawei 已有的 `MindSpeed core_r0.16.0` 拉起 miles | TODO-priority | — | **2026-05-28 GAME-CHANGER**:之前认定「MindSpeed 不支持 Mcore 0.16」**错了**。仔细 grep MindSpeed 分支(之前只看 master + 4 个 release),漏看了 `origin/core_r0.14.0` / `origin/core_r0.15.3` / **`origin/core_r0.16.0`**。该 0.16.0 分支 README 公告 **"May 11, 2026: 🚀 MindSpeed Core 支持Mcore 0.16.0版本"**,Huawei 团队就在今天(2026-05-28 17:10 北京时间)还在 push 「(feat):dsa support TND format」。已有关键 commits:`adapt DSA to core_r0.16.0` (2026-04-30)、`MLATransformerConfig patch`、`kvallgather/ulysses adaptor 016 te`、`adaptor for megatron bridge`、`fix: swap optimizer/dsa` 等。**结论**:**不要做兼容分支**,直接用 Huawei `core_r0.16.0`。**子任务**:**T13.A** — tlrescue 切 `core_r0.16.0` 重装,跑 `import mindspeed.megatron_adaptor` 看在 `Megatron-LM-miles`(Mcore 0.16.0rc0)上 import 通不通。**T13.B** — 视 A 结果:miles `_e2e_megatron_step.py` 删 manual monkey-patch、改成 `import mindspeed.megatron_adaptor`,跑 `MILES_E2E_SHAPE=real`,看能否拉起 DSAMLASelfAttention。开 DSA fused-op flags(`use_fused_lightning_indexer / use_fused_sparse_flash_attention / use_fused_lightning_indexer_kl_loss`),看 Huawei DSA op(`npu_lightning_indexer.cpp` + `triton_indexer_bf16.py`)能否直接驱动 miles。若 yes → 我们 tilelang 4 算子 production value 大幅下降。**T13.C** — 若有缺(R-KA-16 或某 OPEN issue 触发),把 T32 成果(尤其 R-KA-16 bisect)做精确诊断 commit 提 PR 到 `Ascend/MindSpeed core_r0.16.0`,而不是搞自己的 compat fork。a5_ops 算子生成不需要(Huawei DSA 已有 C++ + Triton 实现)。→ artifact:T13.A import log / T13.B miles e2e PASS log / T13.C MindSpeed PR(若需要) |
