...
| **T12** | 4 个上游 PR 整理 + 提交 | IN-PROGRESS | T10, T11 | 4 个 PR 拆解:**(1) `tile-ai/tilelang-mlir-ascend` PR #80** 已开,等 test CI + reviewer。**(2) `Ascend/AscendNPU-IR`**:T9 已用 issue #251 comment 路径替代上游 PR(Huawei 偏好 issue 沟通,改 C++ 由编译器组接手)。**(3) Megatron-LM 同名问题不在 `NVIDIA/Megatron-LM` 上游 main(grep 确认 main 已无 `te_general_gemm` 引用),只在 `radixark/Megatron-LM` miles-main 分支(单 commit `23924a0`)有**;本地已在 `Megatron-LM-miles` checkout 上 branch `fix/te_general_gemm_npu_fallback` 准备好 commit `6f3209b`(8 行 patch:在 `except ImportError` 后绑 `te_general_gemm = None`),本地 cold import 通过验证;等用户确认 fork 目标(radixark vs upstream)再 push 开 PR。**(4) `radixark/miles`**:`_npu/` 子包 + dispatcher 已在 fork `npu-tilelang-dispatch` 分支,需 cleanup → 抽 dispatch hook + 文档 + 提 PR。无 Claude 签名,gc CLI 发。→ artifact:4 PR URL |
