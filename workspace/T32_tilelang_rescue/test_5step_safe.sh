...
set -u
LOG=/home/z00637938/workspace/test_5step_smoke.log
WDLOG=/home/z00637938/workspace/test_5step_wd.log
> "$LOG"; > "$WDLOG"

docker exec -d sgl_probe bash -c "cd /tmp && python /home/z00637938/workspace/test_5step_weight_sync.py > $LOG 2>&1"
...
while true; do
    ...
    if (( EL > 600 )); then echo "[wd] TIMEOUT" | tee -a "$WDLOG"; docker exec sgl_probe bash -c "pkill -9 -f test_5step_weight_sync || true"; break; fi
    if [ -z "$CHIP" ]; then
        for c in 0 1 2 3 4 5 6 7; do
            i=$((c/2)); j=$((c%2))
            if /usr/local/sbin/npu-smi info -t proc-mem -i $i -c $j 2>&1 | grep -q "Process id:$PID"; then
    ...
    if [ -n "$CHIP" ]; then
        ...
        if (( OTH > 50000 )); then echo "[wd] OTHER_USER" | tee -a "$WDLOG"; docker exec sgl_probe bash -c "pkill -9 -f test_5step_weight_sync || true"; break; fi
        if (( TOT > 60000 )); then echo "[wd] TOT_HIGH" | tee -a "$WDLOG"; docker exec sgl_probe bash -c "pkill -9 -f test_5step_weight_sync || true"; break; fi
    fi
    sleep 3
done
