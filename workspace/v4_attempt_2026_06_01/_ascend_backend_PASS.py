...
class AscendAttnBackend(AttentionBackend):
    ...
    def _maybe_upgrade_forward_metadata(self, *args, **kwargs):
        """V4 hook stub: no-op on NPU.

        sglang V4 calls this hook on attention backends to upgrade metadata
        for hash-coding KV layout etc. The NPU backend has no V4-specific
        metadata to upgrade for the bf16 reduced-fab PoC path, so this is
        a no-op. If V4 production needs this hook to do something on NPU,
        upstream sgl-kernel-npu must implement it.
        """
        return None

    def forward_c4_indexer(self, *args, **kwargs):
        """V4 C4 indexer hook stub: no-op on NPU.

        The V4 indexer is invoked from MQALayer._forward_prepare. NPU bf16
        reduced-fab PoC path: skip the indexer routing (we don't have the
        fp8/Hadamard fused kernel + paged indexer write). The full attention
        will fall back to dense over the local tokens.
        """
        return None

    def forward_compressor(self, *args, **kwargs):
        """V4 compressor hook stub: no-op on NPU."""
        return None

    def forward_core_compressor(self, *args, **kwargs):
        """V4 core-compressor hook stub: no-op on NPU.
        Compressor writes intermediate KV-compression state; for the bf16
        reduced-fab PoC we route through the full attention path."""
        return None

    def store_cache(self, *args, **kwargs):
        """V4 store_cache hook stub: no-op on NPU."""
        return None

    def init_forward_metadata_indexer(self, *args, **kwargs):
        """V4 init_forward_metadata_indexer hook stub: no-op on NPU."""
        return None

    def forward_compress(self, *args, **kwargs):
        """V4 forward_compress hook stub: no-op on NPU."""
        return None
    ...
    def forward_extend(
        ...
        **kwargs,  # absorb V4-specific kwargs like compress_ratio, hc_mult
    ):
        ...
        # V4 PoC dense short-circuit: V4 marks itself MHA but passes
        # compress_ratio kwarg + uses an unbuilt V4 KV-pool API. For the
        # bf16 1-layer reduced-fab PoC we compute SDPA directly on fresh
        # q,k,v (no pool reads, no FP8 packed cache). This won't be correct
        # past the first chunked step but lets us validate end-to-end shapes.
        if "compress_ratio" in kwargs and k is not None and v is not None:
            import torch
            seq_lens_cpu = forward_batch.extend_seq_lens_cpu
            # q/k/v come in as [N, n_q_heads, head_dim] / [N, n_kv_heads, head_dim]
            # n_kv_heads=1 for V4. We'll do per-segment causal dense attn.
            outs = []
            offset = 0
            scale = layer.scaling
            for q_len in seq_lens_cpu:
                q_seg = q[offset:offset + q_len]               # [q_len, H, D_q]
                k_seg = k[offset:offset + q_len]               # [q_len, 1, D_k]
                v_seg = v[offset:offset + q_len]               # [q_len, 1, D_v]
                # broadcast k,v over heads
                H = q_seg.shape[1]
                k_b = k_seg.expand(-1, H, -1)
                v_b = v_seg.expand(-1, H, -1)
                # [H, q_len, D]
                qh = q_seg.transpose(0, 1).float()
                kh = k_b.transpose(0, 1).float()
                vh = v_b.transpose(0, 1).float()
                attn = (qh @ kh.transpose(-1, -2)) * scale
                # causal mask
                mask = torch.full((q_len, q_len), float("-inf"), device=q.device)
                mask = torch.triu(mask, diagonal=1)
                attn = attn + mask
                p_attn = torch.softmax(attn, dim=-1)
                out = (p_attn @ vh).transpose(0, 1)            # [q_len, H, D_v]
    ...
    def forward_decode(
        ...
        **kwargs,
        ...
        # V4 PoC decode short-circuit
        if "compress_ratio" in kwargs and k is not None and v is not None:
            import torch
            scale = layer.scaling
            H = q.shape[1]
            qf = q.transpose(0, 1).float()
            kf = k.expand(-1, H, -1).transpose(0, 1).float()
            vf = v.expand(-1, H, -1).transpose(0, 1).float()
            attn = (qf @ kf.transpose(-1, -2)) * scale
            p_attn = torch.softmax(attn, dim=-1)
            out = (p_attn @ vf).transpose(0, 1).to(q.dtype)
            return out
        if is_mla_preprocess_enabled() and self.use_mla:
