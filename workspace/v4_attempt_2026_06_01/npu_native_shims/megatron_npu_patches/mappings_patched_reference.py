...
class _CopyToModelParallelRegion(torch.autograd.Function):
    ...
    NPU/V4 support (blue 2026-06-01): added `all_reduce_grad_fp32` — radixark
    DeepSeek-V4 (deepseek_v4.py:292) calls copy_to_tensor_model_parallel_region
    with this kwarg (present in a newer Megatron-LM than this checkout). When True,
    the backward grad all-reduce is done in fp32 (cast up, reduce, cast back) for
    precision; default False = legacy behavior (no signature change for old callers)."""
    ...
    def backward(ctx, grad_output):
        """Backward function."""
        if getattr(ctx, "all_reduce_grad_fp32", False) and grad_output.dtype != torch.float32:
            orig = grad_output.dtype
            return _reduce(grad_output.float(), ctx.group).to(orig), None, None
        return _reduce(grad_output, ctx.group), None, None
...
def copy_to_tensor_model_parallel_region(input_, group=None, all_reduce_grad_fp32=False):
    ...
    `all_reduce_grad_fp32` (blue 2026-06-01, V4/NPU support): do the backward grad
    all-reduce in fp32 when True (radixark DeepSeek-V4 passes this; newer Megatron-LM has it)."""
