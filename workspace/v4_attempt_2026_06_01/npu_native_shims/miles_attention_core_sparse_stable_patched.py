...
def sparse_attn_torch(q, kv, attn_sink, topk_idxs, sm_scale=None):
    ...
    # NPU/stability (blue 2026-06-01): a query row whose topk are all masked gives
    # scores_max=-inf -> exp(scores - -inf)=nan -> nan grads in backward. Clamp the
    # max to a finite floor so all-masked rows produce 0 weights, not nan.
    scores_max = torch.nan_to_num(scores_max, neginf=0.0)
    exp_scores = torch.exp((scores - scores_max.unsqueeze(-1)).clamp(max=30.0))

    numerator = torch.einsum("bmhk,bmkd->bmhd", exp_scores, kv_gathered.to(torch.float32))

    sum_exp = exp_scores.sum(dim=-1)
    sink_term = torch.exp((attn_sink.view(1, 1, h) - scores_max).clamp(max=30.0))
