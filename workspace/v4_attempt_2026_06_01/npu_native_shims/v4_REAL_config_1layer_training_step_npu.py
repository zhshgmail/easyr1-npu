"""REAL DeepSeek-V4 config reduced to 1 layer on NPU (hidden=4096, 64 heads, MLA, 256-expert MoE)."""
...
print(f"REAL V4 config reduced to 1 layer: hidden={cfg.hidden_size} heads={cfg.num_attention_heads} experts={cfg.num_moe_experts} q_lora={cfg.q_lora_rank} factor={cfg.rotary_scaling_factor}")
...
try:
    ...
    print(f"REAL V4 1-layer block built: {sum(p.numel() for p in block.parameters())/1e9:.2f}B params")
    ...
    print(f"REAL V4 1-LAYER FORWARD OK on NPU: out={tuple(o.shape)} finite={torch.isfinite(o.float()).all().item()}")
...
try:
    ...
    print(f"REAL V4 1-LAYER BACKWARD OK on NPU: loss={loss.item():.4f} grad_norm={gn:.3e} params_with_grad={ng}/{npar}")
    print("=> REAL DSV4 (hidden=4096,64h,256-expert MoE,MLA) reduced-1-layer FULL TRAINING STEP runs on NPU")
except Exception as e:
